...
public class DragableSpace extends ViewGroup {
    ...
    private ArrayList<SnapListener> snapListeners = new ArrayList<SnapListener>();
	private boolean inIgnoreState = false;
    ...
    public void setInIgnoreState(boolean ignore){
    	this.inIgnoreState  = ignore;
    }
    ...
    public boolean isInTouchState(){
    	if (mTouchState != TOUCH_STATE_REST){
    		Log.d(LOG_TAG, "in touch mode");
    		return true;
    	}
    	Log.d(LOG_TAG, "not in touch mode");
    	return false;
    }
    ...
    public void snapToScreen(int whichScreen) {         
        ...
        mScroller.startScroll(mScrollX, 0, delta, 0, 1000);
        invalidate();
        doAfterSnap();
    }
    ...
    public void computeScroll() {
        if (justRotated){
    		mScroller.setFinalX(mCurrentScreen * getWidth());
    		justRotated = false;
	...
	public void addSnapListener(SnapListener sl){
		this.snapListeners.add(sl);
	}
	
	public void removeSnapListener(SnapListener sl){
		this.snapListeners.remove(sl);
	}
	
	public void doAfterSnap(){
		for (SnapListener sl : this.snapListeners){
			sl.afterSnap(this.mCurrentScreen);
		}
	}
